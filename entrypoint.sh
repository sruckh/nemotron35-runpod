...
# 1. apt: audio libs (idempotent)
if ! command -v ffmpeg >/dev/null 2>&1 || ! ldconfig -p 2>/dev/null | grep -q libsndfile; then
  log "installing ffmpeg + libsndfile1"
...
if ! python -c "import nemo.collections.asr" 2>/dev/null; then
  ...
  python -m pip install -q "git+${NEMO_REPO}@${NEMO_REF}#egg=nemo_toolkit[asr]"
else
